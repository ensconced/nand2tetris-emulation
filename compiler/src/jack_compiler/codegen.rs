...
use super::{
    jack_node_types::{
        ASTNode, BinaryOperator, Class, ClassVarDeclaration, ClassVarDeclarationKind, Expression,
...
impl CodeGenerator {
    fn record_vm_commands(&mut self, vm_commands: Vec<Command>, jack_node_idx: usize) {
        for vm_command in vm_commands {
            ...
            self.sourcemap
                .record_vm_command(vm_command_idx, jack_node_idx);
    ...
    fn compile_subroutine_var_declarations(
        &mut self,
        var_declarations: &[ASTNode<VarDeclaration>],
    ) -> usize {
        let mut count = 0;
    ...
    fn compile_return_statement(
        &mut self,
        return_value: &Option<ASTNode<Expression>>,
        return_statement_node_idx: usize,
    ) {
    ...
    fn compile_expression(&mut self, expression: &ASTNode<Expression>) {
        match &*expression.node {
            ...
            Expression::ArrayAccess { var_name, index } => {
                self.compile_array_access_expression(var_name, index, expression.node_idx)
            }
            Expression::Binary { operator, lhs, rhs } => {
                self.compile_binary_expression(operator, lhs, rhs, expression.node_idx)
            }
            Expression::PrimitiveTerm(primitive_term) => {
                self.compile_primitive_term_expression(primitive_term, expression.node_idx)
            }
            Expression::SubroutineCall(subroutine_call) => {
                self.compile_subroutine_call_expression(subroutine_call)
            }
            Expression::Unary { operator, operand } => {
                self.compile_unary_expression(operator, operand, expression.node_idx)
            }
    ...
    fn compile_statement(&mut self, statement: &ASTNode<Statement>) {
        match &*statement.node {
            ...
            Statement::Return(expression) => {
                self.compile_return_statement(expression, statement.node_idx)
            }
            Statement::While {
                condition,
                statements,
            } => self.compile_while_statement(condition, statements, statement.node_idx),
        }
    }
    ...
    fn implicit_return(
        ...
        subroutine_declaration_node_idx: usize,
    ) {
    ...
    fn compile_subroutine(
        &mut self,
        subroutine_declaration: &ASTNode<SubroutineDeclaration>,
        instance_size: usize,
    ) {
        ...
        let locals_count =
            self.compile_subroutine_var_declarations(&subroutine.body.node.var_declarations);
    ...
    pub fn compile_var_declarations(
        &mut self,
        var_declarations: &Vec<ASTNode<ClassVarDeclaration>>,
    ) -> usize {
