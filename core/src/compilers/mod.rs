...
pub fn compile_to_machine_code(sourcemap: &mut SourceMap, jack_code: Vec<&SourceModule>) -> Vec<String> {
    ...
    let parsed_vm_modules: Vec<_> = std_lib_source
        ...
        .map(|source_module| {
            ...
            let class = parse(&source_module.filename, &tokens, sourcemap);
            let commands = generate_vm_code(&source_module.filename, class, sourcemap);
            CompiledJackFile {
